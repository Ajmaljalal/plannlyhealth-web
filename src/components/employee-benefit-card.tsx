...
const BenefitCard = ({ benefit }: any) => {
  ...
  const generalStyle = `relative flex flex-col gap-4 w-full min-w-[290px] min-[781px]:max-w-[364px] min-h-[420px] rounded-[32px] bg-basic_grey_5 text-basic_black`
  ...
  const enrollBtnText = benefit.enrolled ? 'Enrolled' : 'Enroll'
  ...
  return (
    <div className={style}>
      <Image src={images[benefit.title] || '/illustrations/allowances.svg'} width={105} height={132} alt={""} style={{ borderRadius: '24px' }} className="w-full " />
      <div className="w-full px-[16px] pb-[16px] flex flex-col flex-1 justify-between">
        <div className="flex items-center justify-between">
          <h4>{benefit.title}</h4>
          {benefit.enrolled ? <span className="text-basic_white text-[12px] bg-system_success px-[12px] py-[3px] rounded-[16px]">Enrolled</span> :
        ...
        <p className="mt-[4px] mb-[24px] text-small">{benefit.description}</p>
        <div className="flex gap-2">
          <Button className="w-full" text="Details" isSmallBtn onClick={toggleBenefitDetailsModal} />
          {!benefit.enrolled ? <Button className={`w-full ${enrollBtnStyle}`} isSmallBtn isPrimary text={enrollBtnText} /> : null}
        </div>
      </div>
