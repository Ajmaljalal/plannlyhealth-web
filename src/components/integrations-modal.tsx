...
export const IntegrationsModal = ({
  ...
  const renderIntegrations = () => {
    return integrations.map((integration: any) => {
      const isIntegrationActive = benefit.integration?.name === integration.name
      ...
      return (
        <div key={integration.name} className={integrationStyle}>
          ...
          <Button text={addBtnText} className={`w-[180px] ${addBtnStyle}`} isSmallBtn icon={addBtnIcon} onClick={() => onAddIntegration(integration)} />
        </div>
      )
    })
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleCancel} ctaButton={ctaButton} size='large'>
      <div className={modalContentStyles}>
        <h4>{`Choose your ${benefit.title} integration:`}</h4>
